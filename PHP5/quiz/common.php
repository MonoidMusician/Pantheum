...
class _QUIZ
{
	...
	function last() {
		global $sql_stmts;
		$last = NULL;
		sql_getone($sql_stmts["quiz_id->last"], $last, ["i", &$this->_id]);
		return $last;
	}
	function set_last($last) {
		sql_setone($sql_stmts["quiz_id->last="], $last, ["i", &$this->_id]);
	}
	function type() {
		global $sql_stmts;
		$type = NULL;
		sql_getone($sql_stmts["quiz_id->type"], $type, ["i", &$this->_id]);
		return $type;
	}
	function name() {
		global $quiz_types;
	...
	function completed() {
		global $sql_stmts;
		$completed = NULL;
		sql_getone($sql_stmts["quiz_id->completed"], $completed, ["i", &$this->_id]);
		return $completed;
	}
	function time_started() {
		global $sql_stmts;
		$time_started = NULL;
		sql_getone($sql_stmts["quiz_id->time_started"], $time_started, ["i", &$this->_id]);
		return $time_started;
	}
	function time_finished() {
		global $sql_stmts;
		$time_finished = NULL;
		sql_getone($sql_stmts["quiz_id->time_finished"], $time_finished, ["i", &$this->_id]);
	...
	function add_question($question) {
		global $sql_stmts;
		$questions = NULL;
		sql_getone($sql_stmts["quiz_id->questions"], $questions, ["i", &$this->_id]);
		if (!$questions) $questions = "[]";
		$questions = json_decode($questions);
		$questions[] = $question;
		$questions = json_encode($questions);
		sql_set($sql_stmts["quiz_id->questions="], $questions, ["i", &$this->_id]);
		return TRUE;
	}
	function add_result($result) {
		...
		sql_set($sql_stmts["quiz_id->results="], $results, ["i", &$this->_id]);
		return TRUE;
	}
	function options_n() {
		global $sql_stmts;
		$options_n = NULL;
		sql_getone($sql_stmts["quiz_id->options_n"], $options_n, ["i", &$this->_id]);
	...
	function set_options_n($options_n) {
		...
		sql_set($sql_stmts["quiz_id->options_n="], $options_n, ["i", &$this->_id]);
	}
	function add_score($right,$total) {
		global $sql_stmts;
		sql_exec($sql_stmts["add score"], ["iii", $right, $total, &$this->_id]);
		return TRUE;
	}
	...
	function score() {
		global $sql_stmts;
		$score = NULL;
		sql_getone($sql_stmts["quiz_id->score"], $score, ["i", &$this->_id]);
		return $score;
	}
	function out_of() {
		global $sql_stmts;
		$out_of = NULL;
		sql_getone($sql_stmts["quiz_id->out_of"], $out_of, ["i", &$this->_id]);
		return $out_of;
	}
	function percentage() {
...
function NEWQUIZ($type,$last) {
	global $sql_stmts, $suid;
	if (!$suid) return NULL;
	sql_exec($sql_stmts["user_id,type,last->new in quizzes"], ["isi", $suid, $type, $last]);
	$id = NULL;
	sql_getone($sql_stmts["user_id->last quiz_id"], $id, ["i",$suid]);
	if ($id !== NULL) {
