...
while ($idx === NULL || ($quiz = $type->get_option($idx)) === NULL) {
	if (!$cquiz->options_n() or $cquiz->options_n() === true) {
		...
		$cquiz->set_options_n($opts);
	}
	$idx = $cquiz->pop_option();
	if ($i++ > 12) exit("ran out of indices");
}

if ($s = $cquiz->selections())
	$quiz->merge_selections($s);

$try = NULL; $recurse=0; $reason=NULL;
$try = function() use($quiz,$cquiz,&$try,&$recurse,&$reason) {
	global $OP_MATCHING_CHOICES;
	global $OP_MULTIPLE_CHOICE;
	global $OP_MATCHING;
	global $OP_USER_INPUT;
	global $OP_USER_PARAGRAPH;
	if ($recurse >= QUIZ_MAX_RECURSE) {
		echo json_encode([['text', '<span class="jquiz-incorrect" style="font-size: 8px">could not complete sentence '.$reason.'</span>']]);
		return;
	}
	$recurse += 1;
	...
	foreach ($sentence as $word) {
		if (ISHTML($word)) {
			...
			$cquiz_answers["answer$n"] = $results;
			...
			$cquiz_answers[$answers->name] = $answers->serialize_PHP();
	...
	$refresh();
	$cquiz->add_question($result_json);
	$cquiz->set_answers($cquiz_answers);
