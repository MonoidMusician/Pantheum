<?php
    ...
    if ($M_row['multisession'] == 'f') {
        if ($M_row['currentip'] == $_SERVER['REMOTE_ADDR']) {
            $M_query2 = "UPDATE users SET currentip='' WHERE id='$suid';";
            $M_result2 = $mysqli->query($M_query2);
        }
        
        
        if ($M_row['forcelogout'] != '') {
            $result = explode(',', $M_row['forcelogout']);
            if (($result[1] == $_SERVER['REMOTE_ADDR']) || ($result[1] == '*')) {
                $M_query3 = "UPDATE users SET forcelogout='' WHERE id='$suid';";
