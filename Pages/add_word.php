...
<iframe style="width: calc(100% - 500px); height: 300px; float: right; display:none"></iframe>
...
<div>
	Relations:<br>
	<ul id="relations" style="padding-top: 0px"><li><input placeholder="type"><input placeholder="word" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"></ul>
</div>
...
var transform = æ.mix(
	function(r){
		return (
			r.normalize('NFKD').replace(/([aeiouy]) ̄/ig,"$1\u0304")
			.split("ae").join("æ").split("oe").join("œ")
			.split("ss").join("ß").split("ph").join("φ").split("th").join("θ")
			.replace(/^i(?=[aeiouyæœ]|ā|ē|ī|ō|ū|ȳ)/, "j")
		);
	},
	æ.undouble_vowels,
	æ.x
...
$('#definitions input, #forms input, #relations input').autosizeInput();
...
$('#relations input').on('keyup', add_relation = function(e) {
	...
	if (lock || e.which != 13) return; lock=true; // hack
	//alert(e.which);
	$('#relations input').filter(function() {
		return !$(this).val();
	}).parent().remove();
	$('#relations').append('<li><input placeholder="type"><input placeholder="word" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">').on('keyup', add_relation);
	setTimeout(function() {
		updater('#relations input:last', [], []);
		$('#relations input').last().autosizeInput().prev().trigger('focus').autosizeInput();
		lock = false;
	}, 1);
});
