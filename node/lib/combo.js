...
Permutator.Combo = (function() {
	...
	function mapper(v) {
		if (Array.isArray(v))
			return v.map(a => Array.isArray(a) ? new Combo(a) : a);
		return v;
	}
	function filter(arg) {
		for (let i in arg)
	...
	Combo.prototype.pre = function(...arg) {
		arg = filter(arg);
		if (arg === undefined)
			return this.clear();
		for (let a of arg) {
	...
	Combo.prototype.post = function(...arg) {
		arg = filter(arg);
		if (arg === undefined)
			return this.clear();
		for (let a of arg) {
...
Permutator.PermuteOrder = (function() {
	...
	PermuteOrder.prototype.pre = function(...arg) {
		for (let a of arg) {
			this.data.unshift(a);
	...
	PermuteOrder.prototype.post = function(...arg) {
		for (let a of arg) {
			this.data.push(a);
			this.length *= this.data.length;
			if (!Permutator.issimple(a)) this.length *= a.length;
...
Permutator.Mixed = (function() {
	...
	Mixed.prototype.post = function(...arg) {
		for (let a of arg) {
	...
	Mixed.prototype.post_permute = function(...arg) {
		for (let a of arg) {
			this.c.push(null);
			this.p.push(a);
			this.b.push(false);
